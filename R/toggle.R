...
toggle_mode <- function(mode_opt = NULL) {
  ...
  if (is.null(mode_opt)) {
    new <- dplyr::case_when(
      get_current == "tidyverse" ~ "visualisation",
    ...
    if (mode_opt %in% c("tv", "v", "tm", "s", "d")) {
      mode_opt <- dplyr::case_when(
        mode_opt == "tv" ~ "tidyverse",
