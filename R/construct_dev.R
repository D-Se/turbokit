...
expand_usethis_abbreviation <- function(x) {
  ...
  switch(x[1],
    ...
    "i" = {
      ifelse(x[2] == "c", "issue_close_community", "issue_reprex_needed")
    },
    "l" = "local_project",
...
expand_testthat_abbreviation <- function(x) {
  ...
  switch(x[1],
    ...
    "c" = {
      dplyr::case_when(
        length(out) == 3 & x[2] == "o" ~ "capture_output_lines",
    ...
    "g" = {
      ifelse(x[2] == "w", "gives_warning", "get_reported")
    },
    "h" = "has_names",
    "i" = {
      dplyr::case_when(
        ...
        length(x) == 3 & x[2] == "i" ~ "issue_close_community",
        length(x) == 3 & x[2] == "r" ~ "issue_reprex_needed",
      )
    },
    "l" = expand_testthat_l(x),
    "m" = {
      ifelse(length(out) == 1, "matches", "make_expectation")
    },
    "n" = {
      ifelse(length(out) == 1, "not", "new_expectation")
    },
    "p" = "prints_text",
    "q" = "quasi_label",
    "r" = "run_cpp_tests",
