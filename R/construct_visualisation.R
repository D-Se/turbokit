...
#' Function to transform ggmisc abbreviation into character string
#'
...
.expand_ggmisc_abbreviation <- function(x) {
    ...
    switch(
        ...
        "u" = "unscale", #ggfortify
        "f" = {
            dplyr::case_when(x[2] == "m" ~ "fortify_map", #ggfortify
                             x[2] == "p" ~ ft_pal, #hrbrthemes
            )
        },
        "r" = {
            x <- stringi::stri_c(x, collapse = "")
            dplyr::case_when(x == "rg" ~ "removeGrid", #ggExtra
...
#' Function to translate ggmisc abbreviation starting with a into formatted
#' character string
#'
...
# .expand_ggmisc_stat <- function(x) {
#  stopifnot(length(x) <= 5)
# x <- stringi::stri_c(x, collapse = "")

#switch(x,
#      "ss" = "stat_signif", #ggsignif
#     "sb" = "stat_boxploth", #ggstance
#    "sc" = "stat_counth", #ggstance
#   "sx" = "stat_xdensity" #ggstance
#)
...
#' Function to transform cowplot abbreviation into character string
#'
#' @param x string starting with r.
#' @return Adjusted cursor position in R script
...
#' Function to translate cowplot abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
.expand_cowplot_get <- function(x) {
    ...
    if (length(out) == 2) {
        ...
        out[2] <- dplyr::case_when(x[2] == "x" ~ "x",
                                   x[2] == "y" ~ "y",
                                   x[2] == "p" ~ "panel") #get_plot_component
        out[3] <- ifelse(x[3] == "a", "axis", "component")
    }
    paste0(out, collapse = "_")
}

#' Function to translate cowplot_p abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
...
#' Function to translate cowplot_draw abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
...
#' Function to translate cowplot_stamp abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
...
#' Function to translate cowplot_theme abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
.expand_cowplot_theme <- function(x) {
    ...
    if (length(out) == 2) {
        ...
        out[3] <- dplyr::case_when(out[2] == "half" ~ "open",
                                   x[3] == "g" ~ "grid",
                                   x[3] == "h" ~ "hgrid",
                                   x[3] == "v" ~ "vgrid")
        return(paste0(out, collapse = "_"))
...
#' Function to transform ggsci abbreviation into character string
#'
#' @param x string starting with r.
#' @return Adjusted cursor position in R script
...
#' Function to translate ggsci abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
...
#' Function to translate ggthemes abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
...
#' Function to transform ggforce abbreviation into character string
#'
#' @param x string starting with r.
#' @return Adjusted cursor position in R script
.expand_ggforce_abbreviation <- function(x) {
    ...
    switch(
        ...
        "n" = "n_pages",
        "p" = {
            dplyr::case_when(x[2] == "a" ~ "position_auto",
...
#' Function to translate ggforce abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
.expand_ggforce_facet <- function(x) {
    ...
    out[1] <- "facet"
    if (length(out) == 2) {
        out[2] <- dplyr::case_when(x[2] == "c" ~ "col",
                                   x[2] == "m" ~ "matrix",
                                   x[2] == "r" ~ "row",
                                   x[2] == "s" ~ "stereo",
                                   x[2] == "z" ~ "zoom")
...
#' Function to transform ggridges abbreviation into character string
#'
#' @param x string starting with r.
#' @return Adjusted cursor position in R script
.expand_ggridges_abbreviation <- function(x) {
    ...
    switch(
        ...
        "p" = {
            dplyr::case_when(x[2] == "r" ~ "position_raincloud",
...
#' Function to translate ggridges abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
.expand_ggridges_geom <- function(x) {
    ...
    if (length(out) == 2) {
        ...
        if (x[2] == "d") {
            out[2] <- "density"
            out[3] <- dplyr::case_when(x[3] == "l" ~ "lines",
                                       x[3] == "g" ~ "gradient",
                                       x[3] == "r" ~ "ridges")
        } else if (x[2] == "r") {
            out[2] <- "ridgeline"
    ...
    } else if (length(out) == 4) {out <- "geom_density_ridges_gradient"}
    paste0(out, collapse = "_")
}
...
.expand_ggridges_scale <- function(x){
    ...
    out <- character(length(x))
    if (length(out) == 3) {
        if (x == "svl") return("scale_vline_linetype")
        if (x == "sps") return("scale_point_shape")
        out[1] <- "scale"
        out[3] <- "cyclical"
        out[2] <- sub(x = x[2],
                      pattern = x[2],
...
#' Function to transform ggraph abbreviation into character string
#'
#' @param x string starting with r.
#' @return Adjusted cursor position in R script
.expand_ggraph_abbreviation <- function(x) {
    ...
    switch(
        ...
        "a" = "autograph",
        "c" = {
            dplyr::case_when(length(out) == 1 ~ "circle",
        ...
        "f" = {
            dplyr::case_when(x[2] == "e" ~ "facet_edges",
                             x[2] == "g" ~ "facet_graph",
                             x[2] == "n" ~ "facet_nodes")
        },
        "g" = .expand_ggraph_geom(x),
        "i" = "is.geometry",
        "l" = {ifelse(length(x) == 2, "label_rect","layout_to_table")},
        "n" = {ifelse(length(x) == 2, "node_angle", "node_rank_fabric")},
        "r" = "rectangle",
        "s" = {ifelse(length(x) == 1, "square", .expand_ggraph_scale(x))},
        "t" = {dplyr::case_when(length(x) == 2 & x[2] == "f" ~ "th_foreground",
...
#' Function to translate ggraph abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
.expand_ggraph_geom <- function(x) {
    ...
    out <- character(length(x))
    out[1] <- "geom"
    if (x[2] == "a") {return("geom_axis_hive")}
    if (x[2] == "c") {return("geom_conn_bundle")}
    if (x[2] == "e") {
        out[2] <- "edge"
        out[3] <-  sub(x = x[3],
                       pattern = x[3],
...
.expand_ggraph_scale <- function(x) {
    ...
    if (x[2] == "e") {
        ...
        if (length(out) == 3) {return("scale_label_size")}
        x <- stringi::stri_c(x, collapse = "")
        switch(x,
...
#' Function to transform scales abbreviation into character string
#'
#' @param x string starting with r.
#' @return Adjusted cursor position in R script
.expand_scales_abbreviation <- function(x) {
    ...
    switch(
        x[1],
        "a" = {
            dplyr::case_when(length(out) == 1 ~ "alpha",
                             x[2] == "a" ~ "abs_area",
                             x[2] == "p" ~ "area_pal",
                             x[2] == "t" ~ "area_trans") #asn_trans atanh_trans
        },
        "b" = {
            x <- stringi::stri_c(x, collapse = "")
            switch(x,
                    "bt" = "boxcox_trans", "be" = "breaks_extended",
                   "bl" = "breaks_log", "bp" = "brewer_pal", # breaks_pretty
                   "bw" = "breaks_with")
        },
        "c" = .expand_scales_c(x),
        "d" = .expand_scales_d(x),
        "e" = {
            dplyr::case_when(x[2] == "r" ~ "expand_range",
        ...
        "m" = .expand_scales_m(x),
        "n" = {
            dplyr::case_when(length(out) == 1 ~ "number",
...
#' Function to translate scales abbreviation starting with a into formatted
#' character string
#'
#' @param x character vector, maximum length 2.
#' @return full length character string of input.
...
.expand_scales_m <- function(x) {
    stopifnot(length(x) <= 5)
    out <- character(length(x))
    dplyr::case_when(length(out) == 1 ~ "muted",
